...
if (!empty($_POST)) {
    ...
    if (empty($_POST['pseudo']) || empty($_POST['mdp'])) { // On vérifie que pseudo et mdp sont vides // s'il sont vides, on met un message d'erreur grâce à la variable $contenu
    ...
    if (empty($contenu)) { // si la variable $contenu est vide c'est que je n'ai pas d'erreur, je peux donc lancer la connexion 
        $resultat = $pdoBlog->prepare('SELECT * FROM users WHERE pseudo = :pseudo'); // je sélectionne toutes les infos de l'utilisateur dont le pseudo correspond à celui du formulaire
        ...
        if ($resultat->rowCount() == 1) { // si le programme renvoie une ligne c'est que le membre (le pseudo) existe
            ...
            if (password_verify($_POST['mdp'], $membre['mdp'])) { // password_verify prend deux arguments : 1- le mot de passe du formulaire // 2- Le mot de passe de la BDD // password_verify permet de vérifier que le premier correspond au deuxième
                $_SESSION['users'] = $membre; // on créé une session  avec les infos de l'utilisateur dans un tableau multidimensionnel
                // une fois que la session a été créée , on redirige vers la page index
